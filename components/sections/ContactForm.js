      ...
      <div class="contact-grid">
        ...
        <div class="fade-in">
          ...
          <div class="contact-links" style="margin-top:2rem;">
            ...
            <a class="contact-link" :href="cvUrl" target="_blank">
              CV <span>Télécharger →</span>
            </a>
          </div>
        </div>
        ...
        <div class="fade-in">
          ...
          <form v-else @submit.prevent="envoyer" class="contact-form">
            <div class="form-group">
              ...
              <input v-model="nom" type="text" class="form-input" :class="{ error: erreurs.nom }" :placeholder="T.contact.nom"/>
              <span v-if="erreurs.nom" class="form-error">{{ erreurs.nom }}</span>
            </div>
            <div class="form-group">
              <label class="form-label">{{ T.contact.email }}</label>
              <input v-model="email" type="email" class="form-input" :class="{ error: erreurs.email }" placeholder="votre@email.com"/>
              <span v-if="erreurs.email" class="form-error">{{ erreurs.email }}</span>
            </div>
            <div class="form-group">
              <label class="form-label">{{ T.contact.message }}</label>
              <textarea v-model="message" class="form-input" :class="{ error: erreurs.message }" rows="5"></textarea>
              <span v-if="erreurs.message" class="form-error">{{ erreurs.message }}</span>
            </div>
            <button type="submit" class="btn-nav btn-primary" style="width:100%;" :disabled="loading">
